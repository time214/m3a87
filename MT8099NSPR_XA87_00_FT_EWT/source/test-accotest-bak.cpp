...
enum Testmode	{ 
	VBG = 1,        // VFB, VBG
	BSTOK,          // BSTOK, FREQ
	VALLEY,         // SW=GND LSRON, VALLEY
	PEAK,           // SW=VIN HSRON, PEAK
	PPT             // PPTrim
};
typedef enum Testmode TM;
TM tmTGT1 = VBG;
TM tmTGT2 = BSTOK;
TM tmTGT3 = VALLEY;
TM tmTGT4 = PEAK;
...
DUT_API int SupplyCurrent(short funcindex, LPCTSTR funclabel)	{
  ...
  // TODO: Add your function code here
	...
	rlyC.SetOn(-1);
	delay_ms(1);
	rlyC.SetOn(CapVIN, CapVCC, CapBST,
						 ViVIN, ViEN, ViFB, -1);
 	delay_ms(2);
	...
	delay_ms(1);
	// FB 1ST
	FB_FOVI8.Set(FV, 1.35f, FOVI_5V, FOVI_100MA, RELAY_ON, 5);
	delay_ms(1);

	VIN_FPVI0.Set(FV, 24.0f, FPVI10_50V, FPVI10_100MA, RELAY_ON);
	delay_ms(25);
	VIN_FPVI0.Set(FV, 24.0f, FPVI10_50V, FPVI10_1MA, RELAY_ON);
	delay_ms(25);
	...
	// ************************************** IQ **************************************
	// VCC
	VCC_FOVI10.Set(FV, 0.0f, FOVI_5V, FOVI_1MA, RELAY_OFF);
	delay_ms(2);
	...
	delay_ms(1);
	// FB
	FB_FOVI8.Set(FV, 0.5f, FOVI_5V, FOVI_1MA, RELAY_ON);
	delay_ms(1);
	FB_FOVI8.Set(FV, 0.6f, FOVI_5V, FOVI_1MA, RELAY_ON);
	delay_ms(1);
	FB_FOVI8.Set(FV, 0.7f, FOVI_5V, FOVI_1MA, RELAY_ON);
	delay_ms(1);
	FB_FOVI8.Set(FV, 0.8f, FOVI_5V, FOVI_1MA, RELAY_ON);
	delay_ms(1);
	FB_FOVI8.Set(FV, 0.9f, FOVI_5V, FOVI_1MA, RELAY_ON);
	delay_ms(1);
	FB_FOVI8.Set(FV, 1.0f, FOVI_5V, FOVI_1MA, RELAY_ON);
	delay_ms(1);
	FB_FOVI8.Set(FV, 1.1f, FOVI_5V, FOVI_1MA, RELAY_ON);
	delay_ms(1);
	FB_FOVI8.Set(FV, 1.2f, FOVI_5V, FOVI_1MA, RELAY_ON);
	delay_ms(1);
...
DUT_API int FuncEN(short funcindex, LPCTSTR funclabel)	{
  ...
  float vv = 0.0f;
	...
	// FB = 3
 	...
 	delay_ms(1);
	// FB
 	FB_FOVI8.Set(FI, -0.1e-6f, FOVI_5V, FOVI_10UA, RELAY_SENSE_ON);
 	delay_ms(1);
	// **************************** VEN_ON, GONOGO ****************************
	if(GNG)	{
	}
	...
	for(site = 0; site < SITENUM; site++)
		IENUP1[site] = EN_FOVI2.GetMeasResult(site, MIRET);
	...
	//rlyC.SetOn(RT500K, FB100, ViFB, CapVCC, CapVIN, ViVIN, ViFB, ViEN, /*SW10KGND,*/ -1);
	//rlyC.SetOn(RT500K, CapVCC, CapVIN, ViSW, ViVIN, SW10KGND, CapBST, /*ViFB,*/ -1);
	rlyC.SetOn(RT500K, CapVCC, CapVIN, CapBST, ViVIN, ViVIN, -1);
 	delay_ms(2);
	
	for(vv = 0.0f; vv <= 12.0f;)	{
		// VIN
 		VIN_FPVI0.Set(FV, vv, FPVI10_50V, FPVI10_10MA, RELAY_ON);
 		delay_us(300);
		vv += float(0.5);
	}
	// EN
	...
	////delay_ms(2);
	// EN = 1.23
	EN_FOVI2.Set(FV, 1.23f, FOVI_5V, FOVI_10MA, RELAY_ON);
	delay_ms(10);

	// SW
	SW2_FPVI3.Set(FI, -0.1e-3f, FPVI10_50V, FPVI10_10MA, RELAY_SENSE_ON);
	delay_ms(2);
	...
	FreshSiteFlagInit();
 	for(vj = 1.2; vj  < 2.8f;)	{
		for(site = 0; site < SITENUM; site++)	{
			...
			SW2_FPVI3.MeasureVI(10, 10);
			TMPV[site] = SW2_FPVI3.GetMeasResult(site, MVRET);
 			if(TMPV[site] > float(2.0))	{
				EN_FOVI2.MeasureVI(10, 10);
...
#endif
	VIN_FPVI0.Set(FV, 0.0f, FPVI10_50V, FPVI10_10MA, RELAY_ON);
	EN_FOVI2.Set(FV, 0.0f, FOVI_5V, FOVI_10MA, RELAY_ON);
	delay_ms(2);
	VIN_FPVI0.Set(FV, 0.0f, FPVI10_50V, FPVI10_10MA, RELAY_OFF);
	EN_FOVI2.Set(FV, 0.0f, FOVI_5V, FOVI_10MA, RELAY_OFF);
	delay_ms(2);
	...
	for(site = 0; site < SITENUM; site++)	{
		IEN1->SetTestResult(site, 0, -1*IENUP1[site]*1e6);
		//IEN2->SetTestResult(site, 0, -1*IENUP2[site]*1e6);
		//IEN2->SetTestResult(site, 0, VC1[site]);
	}
...
DUT_API int HSide(short funcindex, LPCTSTR funclabel)	{
  ...
  double VDS[SITENUM] = { 0.0f }, ILOADING = 750e-3f, RonHS[SITENUM] = { 0.0f };
  ...
  // TODO: Add your function code here
	...
	// FB = 0V
 	FB_FOVI8.Set(FV, 0.65f, FOVI_5V, FOVI_1MA, RELAY_ON);		// 0V, issue
 	delay_ms(1);
 	FB_FOVI8.Set(FV, 0.65f, FOVI_5V, FOVI_10UA, RELAY_ON);
 	delay_ms(1);
  ...
  delay_ms(1);
	...
	// FB = 5V/1MA
	FB_FOVI8.Set(FV, 5.0f, FOVI_5V, FOVI_1MA, RELAY_ON);
 	delay_ms(1);
 	//FB_FOVI8.Set(FV, 5.0f, FOVI_5V, FOVI_10UA, RELAY_ON);
 	//delay_ms(1);
...
////#ifdef DEBUG
	...
	// FB = 0V
 	FB_FOVI8.Set(FV, 0.65f, FOVI_5V, FOVI_1MA, RELAY_ON);		// 0V, issue
 	delay_ms(1);
 	FB_FOVI8.Set(FV, 0.6f, FOVI_5V, FOVI_10UA, RELAY_ON);
 	delay_ms(1);
...
////#endif
	...
	delay_ms(1);
	// VIN=100, FPVI_PLUS
	VIN_FPVI0.Set(FV, 100.0f, FPVI10_100V, FPVI10_100UA/*FPVI10_1MA*/, RELAY_ON);
 	delay_ms(2);
	// SW=0, FPVI_PLUS
	...
	// **************************************** VIN=28, SW=GND LEAKAGE ****************************************
	// VIN=100, FPVI_PLUS
	VIN_FPVI0.Set(FV, 28.0f, FPVI10_100V, FPVI10_100UA, RELAY_ON);//28
 	delay_ms(20);
	...
	for(site = 0; site < SITENUM; site++)	{
		...
		if(TMPV[site] > float(27.5))	{
			//VIN_FPVI0.MeasureVI(200,5);
			//HSLKG1[site] = VIN_FPVI0.GetMeasResult(site, MIRET);
			SW2_FPVI3.MeasureVI(200, 5);
...
DUT_API int LSide(short funcindex, LPCTSTR funclabel)	{
  ...
  //}}AFX_STS_PARAM_PROTOTYPES
  double VDS[SITENUM] = { 0.0f }, ILOADING = 750e-3f, lsron[SITENUM] = { 0.0f };
  ...
  // FB = 5V/10uA
	...
	delay_ms(1);
  // VIN = 24V
	VIN_FPVI0.Set(FV,	float(24), FPVI10_50V, FPVI10_1A, RELAY_ON);
  delay_ms(1);
	// EN = 0
	EN_FOVI2.Set(FV, 0.0f, FOVI_5V, FOVI_1MA, RELAY_ON);
	delay_ms(1);
  ...
  delay_ms(10);
	...
	delay_ms(1);
	// VIN=100, FPVI_PLUS
	VIN_FPVI0.Set(FV, 100.0f, FPVI10_100V, FPVI10_1MA, RELAY_ON);
 	delay_ms(2);
	VIN_FPVI0.Set(FV, 100.0f, FPVI10_100V, FPVI10_100UA, RELAY_ON);
 	delay_ms(2);

	// SW=100, FPVI_PLUS
	SW2_FPVI3.Set(FV, 100.0f, FPVI10_100V, FPVI10_1MA, RELAY_ON);
 	delay_ms(2);
	//SW2_FPVI3.Set(FV, 100.0f, FPVI10_100V, FPVI10_100UA, RELAY_ON);
	//delay_ms(2);

	VIN_FPVI0.MeasureVI(50, 10);
	SW2_FPVI3.MeasureVI(50, 10);
	for(site = 0; site < SITENUM; site++)	{
...
DUT_API int Switching(short funcindex, LPCTSTR funclabel)	{
  ...
  for(site = 0; site < SITENUM; site++)
	...
	// **************************************** OFF TIME ****************************************
	qtmu0.SetStartTrigger(1.6f, QTMU_PLUS_NEG_SLOPE);	
	qtmu0.SetStopTrigger(1.8f, QTMU_PLUS_POS_SLOPE);	
	delay_ms(1);
	qtmu0.Meas(QTMU_PLUS_COARSE, QTMU_PLUS_TRNG_US);
  for(site = 0; site < SITENUM; site++)
...
DUT_API int PPTrim(short funcindex, LPCTSTR funclabel)  {
  //{{AFX_STS_PARAM_PROTOTYPES
  CParam *TB1 = StsGetParam(funcindex,"TB1");						// vbg -4.5mv
  CParam *TB2 = StsGetParam(funcindex,"TB2");						// vbg -9.0mv
  CParam *TB3 = StsGetParam(funcindex,"TB3");						// vbg -8.0mv
  CParam *TB4 = StsGetParam(funcindex,"TB4");						// vbg 36.0mv
  CParam *TB5 = StsGetParam(funcindex,"TB5");						// IPK1 10%
  CParam *TB6 = StsGetParam(funcindex,"TB6");						// IPK2 -10%
  CParam *TB7 = StsGetParam(funcindex,"TB7");						// IQ -5uA
  CParam *TB8 = StsGetParam(funcindex,"TB8");						// Valley1 10%
  CParam *TB9 = StsGetParam(funcindex,"TB9");						// TBTADD, CLKH
  CParam *TB10 = StsGetParam(funcindex,"TB10");					// FPWM
  CParam *TB11 = StsGetParam(funcindex,"TB11");					// BSTCMP
  CParam *TB12 = StsGetParam(funcindex,"TB12");					// DISACLK
  CParam *TB13 = StsGetParam(funcindex,"TB13");					// BSTSLP
  CParam *TB14 = StsGetParam(funcindex,"TB14");					// EABAIS
  CParam *TB15 = StsGetParam(funcindex,"TB15");					// WAIT
  CParam *TB16 = StsGetParam(funcindex,"TB16");					// FREEZE BIT
  //}}AFX_STS_PARAM_PROTOTYPES
  ...
  short jj = 0;
	rlyC.SetOn(RT500K, 
 	...
 	delay_ms(2);
	// VIN=12
	VIN_FPVI0.Set(FV, 12.0f, FPVI10_50V, FPVI10_1A, RELAY_ON);
 	delay_ms(2);
	// EN = 5.0V
	EN_FOVI2.Set(FV, 5.0f, FOVI_5V, FOVI_100MA, RELAY_ON);
	delay_ms(1);
	...
	// EN = 0.0V		// *************************************************** TM1
 	EN_FOVI2.Set(FV, 0.0f, FOVI_10V, FOVI_10MA, RELAY_ON);	// 1ST FALLING EDGE
 	delay_us(100);
	// EN = 5.0V
	EN_FOVI2.Set(FV, 5.0f, FOVI_10V, FOVI_10MA, RELAY_ON);
	delay_us(100);
	// EN = 0.0V		// *************************************************** TM2
 	EN_FOVI2.Set(FV, 0.0f, FOVI_10V, FOVI_10MA, RELAY_ON);	// 2ND FALLING EDGE
 	delay_us(100);
	// EN = 5.0V
	EN_FOVI2.Set(FV, 5.0f, FOVI_10V, FOVI_10MA, RELAY_ON);
	delay_us(100);	
	// EN = 0.0V		// *************************************************** TM3
 	EN_FOVI2.Set(FV, 0.0f, FOVI_10V, FOVI_10MA, RELAY_ON);	// 3RD FALLING EDGE
 	delay_us(100);
	// EN = 5.0V
	EN_FOVI2.Set(FV, 5.0f, FOVI_10V, FOVI_10MA, RELAY_ON);
	delay_us(100);	
  // EN = 0.0V		// *************************************************** TM4
 	EN_FOVI2.Set(FV, 0.0f, FOVI_10V, FOVI_10MA, RELAY_ON);	// 4TH FALLING EDGE
 	delay_us(100);
	// EN = 5.0V
	EN_FOVI2.Set(FV, 5.0f, FOVI_10V, FOVI_10MA, RELAY_ON);
	delay_us(100);
	// EN = 0.0V		// *************************************************** TM5
 	EN_FOVI2.Set(FV, 0.0f, FOVI_10V, FOVI_10MA, RELAY_ON);	// 5TH FALLING EDGE
 	delay_us(100);
	...
	for(site = 0; site < SITENUM; site++) {
      ...
      if (flag[site] < 0.5f)    {
        for(jj = 0; jj <= SITENUM; jj++)  {
          if(Trimbit1[site] == 1 )	packagetrim(1);
          if(Trimbit2[site] == 1)	  packagetrim(2);
	        if(Trimbit3[site] == 1)	  packagetrim(3);
	        if(Trimbit4[site] == 1)	  packagetrim(4);
	        if(Trimbit5[site]== 1)	  packagetrim(5);
	        if(Trimbit6[site] == 1)	  packagetrim(6);
	        //if(Trimbit7[site] == 1)   packagetrim(7);
				  if(Trimbit8[site] == 1)	  packagetrim(8);
				  if(Trimbit9[site] == 1)	  packagetrim(9);
				  if(Trimbit10[site] == 1)	packagetrim(10);
				  if(Trimbit11[site] == 1)	packagetrim(11);
          if(Trimbit12[site] == 1)	packagetrim(12);
          if(Trimbit13[site] == 1)	packagetrim(13);
				  if(Trimbit14[site] == 1)	packagetrim(14);
				  //if(Trimbit15[site] == 1)	packagetrim(15);
					//if(Trimbit16[site] == 1)	packagetrim(16);
					delay_us(100);
				}
				for(jj = 0; jj <= SITENUM; jj++)
          if(Trimbit7[jj] == 1) 	packagetrim(7);

				for(jj = 0; jj <= SITENUM; jj++)
	        if(Trimbit15[jj] == 1)  packagetrim(15);
        
				for(jj = 0; jj <= SITENUM; jj++)
	        if(Trimbit16[jj] == 1)  packagetrim(16);		// freeze
        flag[jj] = 1;
      }
    }
		END_SINGLE_SITE();
  ...
  for(site = 0; site < SITENUM; site++ )  { 
	  TB1  ->SetTestResult(site, 0, Trimbit1[site]);
		TB2  ->SetTestResult(site, 0, Trimbit2[site]);
		TB3  ->SetTestResult(site, 0, Trimbit3[site]);
		TB4  ->SetTestResult(site, 0, Trimbit4[site]);
		TB5  ->SetTestResult(site, 0, Trimbit5[site]);
		TB6  ->SetTestResult(site, 0, Trimbit6[site]);
		TB7  ->SetTestResult(site, 0, Trimbit7[site]);
		TB8  ->SetTestResult(site, 0, Trimbit8[site]);
		TB9  ->SetTestResult(site, 0, Trimbit9[site]);
		TB10 ->SetTestResult(site, 0, Trimbit10[site]);
		TB11 ->SetTestResult(site, 0, Trimbit11[site]);
    TB12 ->SetTestResult(site, 0, Trimbit12[site]);
    TB13 ->SetTestResult(site, 0, Trimbit13[site]);
    TB14 ->SetTestResult(site, 0, Trimbit14[site]);
    TB15 ->SetTestResult(site, 0, Trimbit15[site]);
		TB16 ->SetTestResult(site, 0, Trimbit16[site]);
	}    
	VIN_FPVI0.Set(FV, 0.0f, FPVI10_50V, FPVI10_1A, RELAY_ON);
	EN_FOVI2.Set(FV, 0.0f, FOVI_5V, FOVI_100MA, RELAY_ON);
	FB_FOVI8.Set(FV, 0.0f, FOVI_10V, FOVI_10MA, RELAY_ON);
